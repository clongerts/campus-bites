...
export default function MapPage() {
  ...
  const pinchZoomRef = useRef<any>(null); // Ref to access the zoom controller
  ...
  // Functional Zoom Handlers
  const handleZoomIn = () => {
    if (pinchZoomRef.current) {
      pinchZoomRef.current.scaleTo({
        scale: pinchZoomRef.current.scale * 1.4,
  ...
  return (
    <main className="relative h-screen w-full text-slate-900 font-sans overflow-hidden p-4 md:p-6 lg:p-8">
      ...
      <div className="fixed inset-0 z-0 bg-gradient-to-b from-transparent to-slate-900/50 pointer-events-none" />
        ...
        <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6 pb-2">
          ...
          <div className="lg:col-span-8 relative bg-slate-100 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden border-4 md:border-8 border-white/40 shadow-2xl touch-none h-full">
            <QuickPinchZoom 
              ...
              tapZoomFactor={1.5}
            >
              {/* Added flex utilities to ensure center alignment and no gaps */}
                ...
                {Object.entries(CAMPUS_LOCATIONS).map(([key, loc]) => (
                  ...
                  >
                    <button
                      ...
                      className={`relative group transition-all duration-500 flex flex-col items-center
                        ${selectedStall?.id === loc.id ? "scale-125" : "hover:scale-110"}
                      `}
                    >
                      <span className={`absolute -top-10 px-3 py-1 bg-white shadow-xl rounded-lg text-[10px] font-black uppercase tracking-tighter transition-all duration-300 pointer-events-none whitespace-nowrap
                        ${selectedStall?.id === loc.id ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0"}
                      `}>
                        {loc.name}
                      </span>
                      
                      <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full border-2 md:border-4 border-white shadow-lg flex items-center justify-center transition-colors
                        ${selectedStall?.id === loc.id ? "bg-yellow-400" : "bg-[#003A70]"}
                      `}>
            ...
            <div className="absolute bottom-4 right-4 md:bottom-8 md:right-8 flex flex-col gap-2 z-50">
              <button 
                ...
                className="w-12 h-12 bg-white/90 backdrop-blur-md border border-slate-200 rounded-xl shadow-xl flex items-center justify-center text-2xl font-bold text-[#003A70] hover:bg-white active:scale-90 transition-all"
              >
                +
              </button>
              <button 
          ...
          <div className="lg:col-span-4 h-full min-h-0">
            <aside className={`h-full flex flex-col rounded-[2rem] md:rounded-[2.5rem] transition-all duration-500 shadow-2xl overflow-hidden
              ${selectedStall ? "bg-[#003A70]/95 backdrop-blur-md text-white border border-white/10" : "bg-white/90 backdrop-blur-md border border-white"}
            `}>
              {selectedStall ? (
                <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="p-6 md:p-10 flex-1 overflow-y-auto custom-scrollbar">
                    <div className="inline-block px-3 py-1 rounded-full bg-yellow-400 text-[#003A70] text-[10px] font-black uppercase tracking-widest mb-4">
                      {selectedStall.category}
                    </div>
                    <h2 className="text-2xl md:text-4xl font-black mb-4 leading-tight tracking-tight">
                      {selectedStall.name}
                    </h2>
                    <p className="text-blue-100/90 text-sm md:text-lg leading-relaxed mb-6">
                      {selectedStall.info}
                    </p>
                    <button 
                      onClick={() => setSelectedStall(null)}
                      className="text-[10px] font-black text-yellow-400 hover:text-white transition-colors uppercase tracking-[0.2em]"
                    >
                      ✕ Close Details
                    </button>
                  </div>
                  <div className="p-6 md:p-8 pt-0 space-y-4 flex-none">
                    <Link 
                      href={`/stalls?filter=${selectedStall.id}`}
                      className="group flex items-center justify-center w-full py-4 bg-yellow-400 hover:bg-yellow-300 text-[#003A70] rounded-2xl font-black text-base md:text-lg transition-all shadow-xl"
                      style={{ textDecoration: 'none' }}
                    >
                      View Stalls at {selectedStall.name}
                ...
                <div className="h-full flex flex-col items-center justify-center p-8 text-center">
                  ...
                  <p className="text-slate-500 text-sm font-bold leading-relaxed mb-8">
                    Tap a marker on the map to discover what's inside.
                  </p>
                  <Link 
                    href="/stalls" 
                    className="block w-full py-4 bg-[#003A70] text-white rounded-2xl font-black text-base hover:bg-[#002a50] transition-all shadow-lg"
                    style={{ textDecoration: 'none' }}
                  >
                    Browse All Stalls
                  </Link>
